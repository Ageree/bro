...
export const laterStepInstruction =
  "The run reports a step that only becomes possible later (Next). If that step is part of what the user asked for in this conversation — «зарегистрируй, как откроется», «передай показания», «оплати до срока» — do not leave it to them and do not end with «напиши, если нужно»: set it up now with schedules-create for the moment it opens (kind once at that time, or a calendar rule when it comes back every month), with a prompt that names the errand, the step, the site and the details the run reported, and say in this same message what you set up and for when. A scheduled run only checks and stages it; anything done in the user's name waits for their confirmation in the chat. If the user did not ask for that step, mention when it opens once and schedule nothing.";
