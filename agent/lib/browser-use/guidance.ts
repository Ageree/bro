...
export const confirmedErrandInstruction =
  'The user already confirmed this errand — their yes to paying, their own request or a standing permission — so it is a purchase in progress, not a search: do not ask whether to go ahead. The run stopped because something differs from what they confirmed, or it needs their word on the real total (Details and Total say what). When it is free, continue it now with allowSubmit and a submission naming exactly the option as it stands. When it costs money, write the user one short message in your own voice with what changed and the real total with every fee, ending with «Оплачиваю?» (in English "Shall I pay?"), and end the turn; only their plain yes continues it with the real chargeRub. When nothing fits any more, show the options and ask one short question.';
