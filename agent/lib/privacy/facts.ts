...
function keepAliveVisits() {
  ...
  if (days === 0) return [];
  return [
    `Чтобы вход не пропадал, раз в ${String(days)} дн. облачный браузер сам, без поручения, открывает страницу аккаунта на сайтах, где за последний месяц было поручение и он вошёл (кроме Госуслуг), ничего там не нажимает и закрывает её.`,
  ];
}
