...
const inputSchema = z.object({
  ...
  allowPayment: z
    ...
    .optional()
    .describe(
      "Binds the saved card to the site and its payment processors so the run can pay — a card guarantee that charges nothing today binds it all the same. A submission with chargeRub already carries its payment: there is no approval card. The tool returns one question to send, and only the user's own next reply confirms paying up to the total plus a small margin, for the errand and its follow-ups, so you need not set this. Set it together with withinSpendLimit when the payment may fit the standing spend limit the user set, or when the user already answered the payment question on an errand whose question named no rouble total."
    ),
  withinSpendLimit: z
    .object({
      category: z
 ...
 * an order — rests on the person's own word, not on the model's reading of
 * the conversation: a web page or an email that talks the model into «the
 * user agreed» still meets the card. Paying is not a card. The tool returns
 * one question, and only the person's own reply in a turn they opened
 * confirms it — never text from a page, an email or a browser report, and
 * never an unclear reply. A standing permission they gave for such errands
 * still stands in for the question; the errand's follow-ups carry a
 * confirmation either way. Only a turn the person started can use a standing
 * permission or an earlier confirmation: the turn that reports a browser run
 * reads the page, so there a free submission shows the card and a payment
 * only asks. A scheduled, proactive or background worker has nobody to ask —
 * and its pending input can be answered by the report turn's model — so it
 * is refused outright, standing permission or not.
 */
export async function browserTaskApproval(
  ...
  const replyGate = (facts: PaymentFacts): ApprovalStatus => {
    const spoken = turn === undefined ? undefined : turnWords(context, turn);
    const personTurn = spoken?.personTurn === true;
    const gate = paymentGate({
      facts,
      messages: messages ?? [],
      personTurn,
      said: personTurn ? (turn?.said ?? null) : null,
    });
    return gate.kind === "proceed"
      ? "not-applicable"
      : { reason: gate.reason, type: "denied" };
  };
  const scope = approvalScope(context);
  // Without a workspace nothing the person allowed earlier can be read.
  // A free submission still meets a card. A payment meets the question.
  if (!scope) {
    ...
    const open =
      input.action === "start" && input.allowSubmit === true
        ...
        : [];
    if (open.length > 0) {
      return { reason: unchosenOptionRefusal(open), type: "denied" };
    }
    return callPays(input) ? replyGate(input.submission) : "user-approval";
  }
  const errand =
  ...
  if (consent.by === "card" && callPays(input)) {
    return replyGate(consent.submission);
  }
  return consent.by === "card" ? "user-approval" : "not-applicable";
}

/**
 * Money leaving, or the saved card being bound. A charge of zero is a
 * guarantee on a booking card, not the payment question.
 */
function callPays(input: BrowserTaskInput) {
  const charge = input.submission?.chargeRub;
  return input.allowPayment === true || (charge !== undefined && charge > 0);
}
...
const standingMonthRefusal =
  "Nothing was started: the standing permission's monthly ceiling was used up by another errand a moment ago. Call browser_task again the same way — the tool returns the payment question to send; do not pay before the user answers it.";
  ...
  events: {
    ...
    "step.started": (_event, context) => {
      ...
      const tool = defineTool({
        approval: ({ session, toolInput }) =>
          browserTaskApproval(toolInput, { session }, turn, context.messages),
        description: browserTaskDescription,
        inputSchema,
