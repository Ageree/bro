...
export function classifyNativeLoginControl(
  ...
  if (
    hasPhrase(searchable, "почта") ||
    hasPhrase(searchable, "эл. почта") ||
    hasPhrase(searchable, "электронная почта") ||
    hasPhrase(searchable, "e-mail")
  ) {
    return { ...descriptor, score: 75, token: "email" };
  }
  ...
  if (
    hasPhrase(searchable, "телефон") ||
    hasPhrase(searchable, "моб") ||
    hasPhrase(searchable, "номер телефона")
  ) {
    return { ...descriptor, score: 75, token: "tel" };
  }
  if (
  ...
  if (
    hasPhrase(searchable, "логин") ||
    hasPhrase(searchable, "имя пользователя") ||
    hasPhrase(searchable, "аккаунт")
  ) {
    return { ...descriptor, score: 70, token: "username" };
  }
  return null;
