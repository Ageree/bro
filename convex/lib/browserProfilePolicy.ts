...
Если видишь «Войти» или «Авторизоваться» — нажми, чтобы открылась форма входа (паспорт — нормально).
...
export function loginWaitTask(url: string): string {
  ...
  return `${LOGIN_MARK}
Первым действием открой именно ${page} (сразу navigate, не about:blank и не стартовая Browser Use).
Если уже личный кабинет, имя или заказы — закончи одним словом: вошёл.
Если видишь «Войти» или «Авторизоваться» — нажми, чтобы открылась форма входа (паспорт — нормально).
Дождись формы входа — ссылку человеку отправим только когда эта страница уже на экране.
Дальше жди. Человек может ввести пароль в live-view. Одноразовый код он может прислать в чат — тогда Bro введёт его сам.
Не вводи логин и пароль сам. Пароль в iMessage не проси. Не подтверждай форму входа, пока нет данных.
