...
export const PAY: Journey[] = [
  ...
  {
    name: "Ссылки на подтверждение нет, а фраза всё равно зовёт её открыть",
    group: "pay",
    knownGap:
      "humanLineForNeed('3ds'|'captcha') без liveUrl всё равно говорит «открой ссылку» / «реши по ссылке», хотя ссылки в сообщении нет — человек читает указание открыть то, чего ему не прислали, и застревает. Минимальный фикс: отдельная формулировка без ссылки («банк просит подтвердить оплату у себя в приложении — напиши «готово»»), как это уже сделано для password.",
    steps: [
      {
