...
export const ORDERS: Journey[] = [
  ...
  {
    name: "Длинный номер заказа магазина принимают за карту",
    group: "orders",
    knownGap:
      "Номер заказа из 13–19 цифр (обычная длина для WB/Ozon) отбрасывается как PAN — и строка в orders получает выдуманный pending:id, хотя магазин номер напечатал. «Где заказ» и отмена по этому номеру перестают работать. Минимальный фикс: сверять номер по Луну (как это уже делает secretScrub.looksLikeCardNumber), а не по одной длине.",
    steps: [
      {
