...
/** The shape a real OpenRouter 402 leaves in the session stream. */
function modelCallFailed(): MessageStreamEvent[] {
  return [
    turnStarted(),
    {
      data: {
        code: "MODEL_CALL_FAILED",
        message: openRouterCredits,
        sequence: 1,
        stepIndex: 0,
        turnId,
      },
...
describe("a turn that ends in turn.failed", () => {
  it("records the case as failed, with the code and the start of the message", async () => {
    const fake = await startFakeEve(() => modelCallFailed());
    stopFake = () => fake.close();
    const settings = await settingsFor(fake.url);
    ...
    expect(record.driver.remainingSteps).toEqual([]);
    const saved = await readRunRecord(settings.outDir, "case-under-test");
    expect(saved.driver.status).toBe("failed");
    expect(saved.driver.statusDetail).toBe(
      "MODEL_CALL_FAILED: This request requires more credits, or fewer max_tokens. You requested up to 131072 tokens, but can only afford 24969.…"
    );
    const log = await readFile(
      join(settings.outDir, "case-under-test.log"),
    ...
    expect(log).toContain(
      "!! turn.failed MODEL_CALL_FAILED: This request requires more credits, or fewer max_tokens. You requested up to 131072 tokens, but can only afford 24969. To increase, visit https://openrouter.ai/settings/credits and add more credits"
    );
  });

  it("does not schedule the next scripted step after the failed turn", async () => {
    const fake = await startFakeEve(() => modelCallFailed());
    stopFake = () => fake.close();
    ...
    expect(record.driver.remainingSteps).toEqual([]);
    expect(record.driver.statusDetail).not.toContain("pnpm bench next");
  });
});
