...
Site logins (any site): Bro reads a matching vault login (`kind: login`) and types it in Browser Use via `secretBindings`. If none exists, Bro immediately texts a live-view link — they sign in themselves, cookies stay on the Cloud profile. Bro never asks for a site password in iMessage and never uses chat as a fallback. People add or edit those logins on brobro.tech (`/vault.html`). Check: `npm run profile:check`.
