...
Site logins: Bro texts a link. The person opens it, signs in on the site, and cookies stay on their Browser Use Cloud profile. Bro never sees the password. If a session expires, Bro sends another link. Cloud runs pin `gpt-5.6-luna` with OpenAI's default `reasoning.effort: medium` on errands, `high` on pay, and `none` on login (caps 0.15 / 0.60 / 1.00). Cloud's own Luna default is `xhigh` and would burn tokens; `low` is below both vendor defaults and too weak for marketplace jobs. Runs start proxyless (`proxyCountryCode: null`) except Ozon/Avito, which go to managed RU immediately — those hosts failed from Cloud's AWS IP. If a proxyless page comes back blocked (no connection, IP captcha, SmartCaptcha), Bro starts one RU hop without charging a second browser job. `BRO_BROWSER_PROXY_FALLBACK=0` turns the second hop off. BYOP via `BRO_BROWSER_PROXY_HOST`/`PORT` skips both hops. Check: `npm run profile:check`, `npm run browser:check`.
