...
Turns default to OpenRouter's free router (`openrouter/free`) and then other `:free` models (`BRO_MODEL_FALLBACKS`). Put extra keys you own in `OPENROUTER_API_KEYS` (comma or newline); the fetch wrapper rotates on 401/402/429 and then tries the next model so user messages stay $0 until every free route is exhausted. Keys must be yours — this is not a leaked-key rotator, and free-tier daily limits are usually per OpenRouter account. Set `BRO_MODEL=z-ai/glm-5.3-flash` to pay for the old default. Model calls always carry `max_tokens` (default 8192, `BRO_MAX_OUTPUT_TOKENS`): OpenRouter reserves the requested output length against the account balance before running, and without a cap it reserves the model's full 131k, so every paid turn fails with 402 as soon as credits dip. Postmortem 2026-09-06: `.harness/goals/openrouter-402-rca`. Check: `npm run model:check`.
