...
A per-person computer on Maritime is a persistent microVM (Browserbase live view; a Linux desktop only on a paid plan). Off by default — set `BRO_COMPUTER_BACKEND=maritime` and `MARITIME_TOKEN`. The phone number never goes to Maritime (external id and agent name are a sha256). Check: `npm run computer:check` (add `MARITIME_LIVE=1` to smoke-read plan usage, templates, and an existing agent's live view).
