...
Site logins (any site): Bro reads a matching vault login (`kind: login`) and types it in Browser Use via `secretBindings`. If the Cloud profile already has cookies for that site, Bro does not send another login link. If neither exists, Bro opens the site login page over CDP and then texts a live-view link — they sign in themselves, cookies stay on the Cloud profile. The link is not sent while the preview is blank / about:blank. Bro never asks for a site password in iMessage and never uses chat as a fallback. People add or edit those logins on brobro.tech (`/vault.html`). Check: `npm run profile:check`.
