...
Group chats (Tomo-style): save the Bro vCard and add that number to an iMessage group, or from the 1:1 thread ask Bro to open a chat with 2–8 numbers (`group_chat`). Inkbox groups need a dedicated line (`BRO_DEDICATED_LINE=1`). Shared-pool identities stay 1:1. Bro answers in a group only when addressed (`бро` / `bro`); personal tools (vault, pay, mail, Composio, browser) stay in the private thread. A group never overwrites the tenant's 1:1 `inkboxConversationId`. Check: `npm run group:check`.
