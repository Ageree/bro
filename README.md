...
Site logins use [Browser Use Cloud profile sync](https://docs.browser-use.com/cloud/guides/profile-sync): on the computer where Chrome is already logged in, run `export BROWSER_USE_API_KEY=… && curl -fsSL https://browser-use.com/profile.sh | sh`, then set `BROWSER_USE_PROFILE_ID` (or paste the profile UUID in the cabinet «Входы в сайты» card). Cookies and local storage go to the cloud profile; the agent never sees passwords. Re-run the helper when a site session expires. Check: `npm run profile:check`.
