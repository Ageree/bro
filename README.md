...
Лимиты считаются по рабочему пространству: сообщения — за местный день,
браузерные поручения — за местный месяц. `FREE_MESSAGES_PER_DAY` (30),
`PAID_MESSAGES_PER_DAY` (500), `FREE_BROWSER_RUNS_PER_MONTH` (5),
`PAID_BROWSER_RUNS_PER_MONTH` (60). Местный день и месяц берутся из таймзоны в
Personal Info, по умолчанию `Europe/Moscow`. За лимитом сообщений человек один
раз в день получает сообщение о лимите со ссылкой на оплату, остальные
сообщения тихо отбрасываются; за лимитом поручений `browser_task` не
запускается и возвращает модели объяснение.
