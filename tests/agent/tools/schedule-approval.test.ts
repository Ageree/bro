...
describe("a schedule set up outside the person's own turn", () => {
  ...
  it("shows what the task will do, word for word, and when", () => {
    ...
    expect(card.prompt).toBe(
      [
        "Поставлю задачу по расписанию: Открой онлайн-регистрацию на рейс SU 1124 Москва — Сочи и подготовь место у прохода.",
        "Расписание — 2026-10-02 09:35 (UTC+03:00).",
        "Поставить?",
      ].join("\n")
    );
  });
  ...
  it("names a change and a pause on the card of an update", () => {
    ...
    expect(card.prompt).toBe(
      [
        "Поменяю задачу по расписанию.",
        "Поставлю её на паузу.",
        "Поменять?",
      ].join("\n")
    );
  });
  ...
  it("names a run now on the card, so it does not read as an empty change", () => {
    ...
    expect(card.prompt).toBe(
      [
        "Поменяю задачу по расписанию.",
        "Запущу её один раз сейчас, вне расписания.",
        "Поменять?",
      ].join("\n")
    );
  });
