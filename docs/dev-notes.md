...
- Некоторые модели (в бенчмарке `openai/gpt-6-luna`) в одном ходе шлют
  ...
  биграммам тексты с разными числами, датами, именами или ссылками (рейс туда и
  обратно, «Вариант 1/2») повтором не считаются. Для строкового id Gateway
...
- Подтверждение `spend_limit` нужно всему, что поднимает потолок хоть для
  одной цели: и `clear` правила, над которым стоит более широкое.
