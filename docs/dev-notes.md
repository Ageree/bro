...
- Некоторые модели (в бенчмарке `openai/gpt-6-luna`) в одном ходе шлют
  ...
  седьмое сообщение отбрасывает результатом «не доставлено, заверши ход», а
  следующий шаг идёт с `toolChoice: none`. Для строкового id Gateway остаётся
  только отбрасывание. Параллельные вызовы в одном шаге друг друга не видят.
