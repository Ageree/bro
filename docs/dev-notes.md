...
- Одобренный вызов исполняет AI SDK перед первым шагом модели, и только если
  последнее сообщение истории — `tool` с `tool-approval-response`. Иначе вызов
  молча пропадает: нет `action.result` и ошибки, модель переспрашивает или
  врёт «поставил». Так терялись записи, когда память менялась из другой
  сессии, пока карточка ждала: eve дописывал её после ответа. Чинит патч eve
  (`patches/README.md`), ловит `evals/agent/integrations.eval.ts`; быстрое
  одобрение без смены памяти баг не показывает.
