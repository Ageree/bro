...
- Карточка называет один вариант. Старт с `allowSubmit` и карточкой, где у
  ...
  словам). Поиск без согласия выбирает лучший вариант, доводит до последнего
...
- `schedules-answer` есть только в ходе, начатом человеком, и принимает
  лишь run id, чей вопрос отчёт доставил в этот чат (`shownScheduledQuestions`
  ищет в истории промпт отчёта и успешный `send_message` после него). Ход-отчёт
  больше не отвечает воркеру сам: модель выдумывала ответ за человека.
...
- Инструменты провайдера памяти принимают `approval`, как обычные
  ...
  шага политика друг за другом не видит.
