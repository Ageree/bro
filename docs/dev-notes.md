...
- Язык ответа держит не правило в длинном русском промпте (слабая модель его
  ...
  конце промпта каждого шага (middleware в `agent/lib/model/openrouter.ts`) и
  возврат `send_message` на переписывание (`agent/lib/delivery/language.ts`).
  Отчёт браузера приходит user-сообщением на английском, язык по нему не
  определяется.
...
- Чтения Gmail резолвятся на `step.started` и видят, что ход уже читал
  (`agent/lib/google-workspace/turn-reads.ts`): повтор того же запроса, чтение
  сверх 20 и любое чтение после отказа Google по квоте отвечаются без Google,
  а три таких отказа завершают ход через `toolChoice: none`. Квота приходит и
...
- `gmail-update` больше чем для трёх писем спрашивает карточку, а архивация
